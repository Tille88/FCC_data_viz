document.addEventListener('DOMContentLoaded',function(){
	...
	//console.log(max_val);
	var yScale = d3.scaleLinear().domain([maxVal,0]).range([0, h]);
	...
	var svg = d3.select("body").append("svg").attr("width", w+leftMarg+rightMarg).attr("height", h+lowMarg+upMarg);
	...
	var bars = svg.selectAll(".bar")
					...
					.append("rect")
					  .attr("class", "bar")
					  .attr("x", function(d, i) { return leftMarg + i*w/values.length; })
					  .attr("width", 0.85*w/data.length)
					  //.attr("y", function(d) { return h-yScale(d[1]); })
					  //.attr("height", function(d) { return yScale(d[1]); })
					  .attr("y", function(d) { return upMarg + yScale(d[1]); })
					  .attr("height", function(d) { return h-yScale(d[1]); })
					  .style("opacity", 0);//;
					  //.attr("height", function(d) { return h-xScale(d[1]); });

	
	//svg.call(yAxis)
	var translateText = "translate(" + (leftMarg-5) + "," + upMarg + ")";
  ...
  //.attr("transform"," translate(0,15) rotate(-65)");		
	...
	function tipShow(d) {
		...
		svg.selectAll(".bar")
			.style("opacity", function(d) { return d[0] === chosen[0] ? 1 : .8; });		
			
			
		//from https://stackoverflow.com/questions/16770763/mouse-position-in-d3	
		var mousePos =  d3.mouse(this); //x,y
	
		d3.select("#tooltip")
			.style('top',mousePos[1]+100+"px")
			.style('left',mousePos[0]+220+"px")
			.style('opacity',1)
			.style("color", function(d) {return colorScale(0);});

		
		d3.select("#year-month")
			.html("year-month: " + d[0].slice(0,7));
		d3.select("#gdp-amound")
			.html("GDP (billions &#36;): " + d[1]);	
	...
	bars.on('mouseover', tipShow).on('mouseout', tipHide);
	...
	//yAxis.selectAll("text").style("color", "red");
